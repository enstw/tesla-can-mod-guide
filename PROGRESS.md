...
1. **Order the Adafruit Bluefruit LE UART Friend** for wireless diagnostics, then wire it to the Feather's free hardware UART (`Serial1`; MCP2515 is on SPI) so the `[STATUS]` stream and Section 4 commands are reachable over BLE without opening the trim panel. Do this before sealing the Feather in heat shrink.
1. **Add a NeoPixel status indicator** to the firmware (red = no frames; amber = frames but no target IDs; green = target IDs locked; plus a listen-only vs active distinction) so X179 polarity/pair iteration can be done by glancing at the onboard LED instead of a laptop screen. An LED cannot replace `[STATUS]` (it can't show which CAN ID) — it is a coarse field aid for the 4-combo sweep. Implement alongside the Section 4 work.
...
- **Spec photo is a starting hypothesis only, not wiring confirmation.** Reaffirmed 2026-05-17. The three sources disagree (`pinout-labeled.png` color→bus vs `x170-enhanced.png` pin→Chassis/Body vs hypery11 "Bus 6"), so a photo cannot say which color pair lands on the target bus, and a photo cannot establish H/L polarity at all. Confirmation is target-frame visibility (`0x370`, `0x39B`) in listen-only across up to 4 combos (green/white vs yellow/black × H/L). Consequence for sequencing: firmware-loaded listen-only is the sniffer, so firmware load precedes wiring confirmation, and there is no runtime path from listen-only to active until the Section 4 command interface ships.
...
- **Wireless diagnostics via Adafruit Bluefruit LE UART Friend, not an ESP32 switch.** Decided 2026-05-16. The Feather RP2040 CAN has no onboard radio, so reaching the car-embedded device without opening the trim panel needs an add-on. Switching the permanent install to an integrated hypery11-supported ESP32 board (e.g. LilyGo T-CAN485 — WiFi+BT onboard, no port needed) was evaluated and rejected: it discards the bench-tested Feather power chain and verified TERM cut, and parks an always-on broadcasting radio in a stealth-first build (Taiwan ban-wave detection cluster, paid permanent FSD = large loss on a ban). Chosen path keeps the Feather and bridges its free `Serial1` (MCP2515 is on SPI) to BLE. Bluefruit LE chosen over HC-05/HC-06 because BLE pairs with iOS *and* Android (Bluetooth Classic SPP does not work with iPhones), and it shares the Feather's Adafruit ecosystem (`Adafruit_BluefruitLE_nRF51` library + Bluefruit LE Connect app). BLE is local-only RF and does not change the CAN-side ban-detection posture; the module can be physically unplugged after bring-up so the permanent state stays RF-silent. A separate ESP32 as a bench-only bus-discovery tool was considered optional and not purchased.
